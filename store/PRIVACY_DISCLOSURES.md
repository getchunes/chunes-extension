...
persist them. Apple Music tabs are included only while the local Apple Music
setting is on. For enabled sources, Chunes sends listening presence to Discord.
...
Music's web metadata endpoint for exact square music artwork, or searches the
public iTunes Search API with title/artist for Apple Music tracks. Those
